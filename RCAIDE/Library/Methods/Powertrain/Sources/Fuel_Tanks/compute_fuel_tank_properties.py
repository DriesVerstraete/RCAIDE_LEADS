...
def compute_fuel_tank_properties(tank,state,distributor):
    ...
    if len(D) > 0:
        tank_conditions.fuel_mass_flow_rate  =  distributor_conditions.fuel_mass_flow_rate * tank.fuel_selector_valve.fuel_flow_split_ratio
        dm_l                                 = -tank_conditions.fuel_mass_flow_rate 
        R                                    = np.dot(D,mass_unknowns)[:, 0] - dm_l[:, 0]
        R[0]                                 = mass_unknowns[0] - tank_conditions.fuel_mass[0, 0]
    
        state.residuals.network[tank.tag + '_mass'][:,0] = R
        tank_conditions.fuel_mass[1:, 0]                 = mass_unknowns[1:, 0]
