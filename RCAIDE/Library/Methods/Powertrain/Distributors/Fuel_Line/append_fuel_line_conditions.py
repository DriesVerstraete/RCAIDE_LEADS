...
def append_fuel_line_conditions(fuel_line,segment,network): 
    ...
    manual_ratio = sum(t.fuel_selector_valve.fuel_flow_split_ratio or 0 for t in fuel_line.fuel_tanks)
    auto_tanks   = [t for t in fuel_line.fuel_tanks if t.fuel_selector_valve.fuel_flow_split_ratio is None]
    total_auto_mass = sum(t.fuel.mass_properties.mass for t in auto_tanks)
    remaining_ratio = max(0.0, 1.0 - manual_ratio)

    for t in auto_tanks:
        t.fuel_selector_valve.fuel_flow_split_ratio = (t.fuel.mass_properties.mass / total_auto_mass) * remaining_ratio if total_auto_mass else remaining_ratio / len(auto_tanks)
