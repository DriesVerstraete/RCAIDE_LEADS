...
def set_network_residuals_and_unknowns(mission):
    ...
    for segment in mission.segments: 
        ...
        for network in networks:
            segment.state.unknowns[network.tag]                 = Conditions()
            segment.state.residuals[network.tag]                = Conditions()
            # Lets do only Fuel Tanks for now will figure out the rest later om 
            for fuel_line in network.fuel_lines:   
                segment.state.unknowns[network.tag].fuel_lines      = Conditions()
                segment.state.residuals[network.tag].fuel_lines     = Conditions()               
