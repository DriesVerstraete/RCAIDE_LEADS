    ...
    for j in range(len(curr)):      
        for i in range(len(battery_chemistry)):
            ...
            for segment in results.segments.values(): 
                volts         = segment.conditions.energy.busses['bus'].battery_modules[battery_chemistry[i]].cell.voltage_under_load[:,0] 
...
def mission_setup(analyses,vehicle,battery_chemistry,current,mAh):
    ...
    base_segment.state.numerics.mission_solver.method   = "root_finder" # options: "optimize", "root_finder"
